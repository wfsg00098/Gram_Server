...
class Connect extends Thread {
    ...
    private class FileReceive extends Thread {
        ...
        public void run() {
            try {
                String fileName = Settings.FILE_PATH + username + "_" + dateFormat.format(new Date()) + fileOriginalName;
                ...
                dos.close();
                socket.close();
                Control.Connections.get(receiver).StartFileSend(fileName);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    class FileSend extends Thread {
        ...
        public void run() {
            try {
                ...
                JSONObject json = new JSONObject();
                json.put("type", "ReadyToSend");
                json.put("length", String.valueOf(file.length()));
                SendJSON(json);
                Socket socket = ss.accept();
...
//            Alive.Stop();
    ...
    private void StartFileSend(String fileName) {
        String status = Control.db.GetUserOnline(username);
        if (!status.equals("offline")) {
            new FileSend(fileName).start();
        } else {
            JSONObject js = new JSONObject();
            js.put("type", "unreceivedFile");
            js.put("filename", fileName);
            Control.db.AddUserUnread(username, js.toString());
        }
    }
